...
const isLocalCodexPath = (path: string) => path.startsWith('local-codex:///');
...
export class APIManager {
  ...
  getJSONL = async ({
    ...
    if (isLocalCodexPath(blobURL)) {
      const parsedURL = new URL(blobURL);
      const kind = parsedURL.pathname.replace(/^\//, '') as
        | 'latest-session'
        | 'session-index'
        | 'prompt-history'
        | 'session-list'
        | 'session-open'
        | 'session-projects';
      return this.getLocalCodexJSONL({
        kind,
        offset,
        limit,
        jmespathQuery,
        baseDir: parsedURL.searchParams.get('baseDir') ?? undefined,
        sourceKind: parsedURL.searchParams.get('sourceKind') ?? undefined,
        sessionId: parsedURL.searchParams.get('sessionId') ?? undefined,
        searchQuery: parsedURL.searchParams.get('searchQuery') ?? undefined,
        projectQuery: parsedURL.searchParams.get('projectQuery') ?? undefined,
        folderQuery: parsedURL.searchParams.get('folderQuery') ?? undefined
      });
    }
  ...
  getLocalCodexJSONL = async ({
    ...
    if (baseDir && baseDir.trim() !== '') {
      queryPath.set('baseDir', baseDir.trim());
    }
    if (sourceKind) {
      queryPath.set('sourceKind', sourceKind);
    }
    if (sessionId) {
      queryPath.set('sessionId', sessionId);
    }
    if (searchQuery && searchQuery.trim() !== '') {
      queryPath.set('searchQuery', searchQuery.trim());
    }
    if (projectQuery && projectQuery.trim() !== '') {
      queryPath.set('projectQuery', projectQuery.trim());
    }
    if (folderQuery && folderQuery.trim() !== '') {
      queryPath.set('folderQuery', folderQuery.trim());
    }

    const result = await fetch(
      `${this.apiBaseURL}local-codex-jsonl/?${queryPath.toString()}`,
      requestOptions
    );
    ...
    if (!result.ok) {
      try {
        const errorResponse = (await result.json()) as { detail: string };
        throw new FileError(errorResponse.detail || `${result.status}`);
      } catch (error) {
        if (error instanceof FileError) {
          throw error;
        }
  ...
  qmdSearch = async ({
    ...
    const queryPath = new URLSearchParams();
    queryPath.set('query', query.trim());
    queryPath.set('limit', limit.toString());
    queryPath.set('semantic', semantic ? 'true' : 'false');
    queryPath.set('allMatches', allMatches ? 'true' : 'false');
    if (baseDir && baseDir.trim() !== '') {
      queryPath.set('baseDir', baseDir.trim());
    }
    if (projectName && projectName.trim() !== '') {
      queryPath.set('projectName', projectName.trim());
    }
    if (folderHint && folderHint.trim() !== '') {
      queryPath.set('folderHint', folderHint.trim());
    }

    const result = await fetch(
      `${this.apiBaseURL}qmd-search/?${queryPath.toString()}`,
      {
        method: 'GET',
        credentials: 'include'
      }
